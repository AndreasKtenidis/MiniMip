...
from library.utils.grizzly_aggregator import GrizzlyAggClient
from library.utils.numpy_aggregator import NumpyAggregator
from library.under_development.stats.univariate_statistics import StandardDeviation
...
class Covariance(StatisticalFunction):
    ...
    def compute(self, x: np.array, y: np.array):
        aggregator = NumpyAggregator(self.client)
...
#     def compute(self, x:np.array, y:np.array):
#         aggregator = NumpyAggClient(self.client)
#         avg_data1 = aggregator.global_avg(x)
#         avg_data2 = aggregator.global_avg(y)
...
class CovariancePandas(StatisticalFunction):
    def compute(self, data: DataFrame, *, x, y):
        aggregator = PandasAggClient(self.client)
...
class CovarianceGrizzly(StatisticalFunction):
    def compute(self, data:DataFrame,*, x,y):
        aggregator = GrizzlyAggClient(self.client)
