...
class BlobDataset2(AbstractTable):
    ...
    def get_attributes(self, *attributes: str):
        ...
        if self.partition_id + 1 == self.num_partitions:
            return pd.DataFrame(BlobDataset2.records[_part_start:, ind],columns=attributes)
        else:
            return pd.DataFrame(BlobDataset2.records[_part_start:_part_start + _part_size, ind][:_part_size],columns=attributes)
    ...
    def get_attribute(self, attribute):
        ...
        _part_start = self.partition_id*_part_size.astype(int)
        if self.partition_id+1==self.num_partitions:
            return pd.DataFrame(BlobDataset2.records[_part_start:, ind])
        else:
            return pd.DataFrame(BlobDataset2.records[_part_start:_part_start + _part_size, ind][:_part_size])
