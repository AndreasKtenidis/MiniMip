...
class CalibrationBelt(AggFunc):
    ...
    def compute(self, e, o, confidence=0.95, max_poly_degree=5):
        ...
        for m in range(1, max_poly_degree + 1):
            ...
            x_vec = add_constant(np.column_stack([g_e ** i for i in range(m + 1)]))
            model=Fed_GLM(self.client)
            model.train(input=x_vec,output=o)
        ...
        x_range = add_constant(np.column_stack([g_e_range ** i for i in range(best_m + 1)]))
        p_pred = best_model.predict(x_range)

        # Step 4: Compute confidence band
        agg = self.get_numpy_aggregator()
        cov_matrix = best_model.cov_params()
        _sum = agg.global_sum([x_range[:, i] * x_range[:, j] * cov_matrix[i, j]
                             for i in range(best_m + 1) for j in range(best_m + 1)])
        print(_sum)
        se = np.sqrt(agg.global_sum([x_range[:, i] * x_range[:, j] * cov_matrix[i, j]
                             for i in range(best_m + 1) for j in range(best_m + 1)]))
        chi2_val = chi2.ppf(confidence, df=2)
        g_p_lower = best_model.predict(x_range) - np.sqrt(chi2_val) * se
        g_p_upper = best_model.predict(x_range) + np.sqrt(chi2_val) * se
