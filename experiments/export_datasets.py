...
for client_id in range(0,3):
    for sample_size in [1000,100000]:
        blob_dataset = BlobDataset(n_samples=sample_size, centers=3).get_local_dataset(client_id, 3).values
        np.savetxt(f'/data/dataset/csvs/blob_dataset{sample_size}_{client_id}.npy', blob_dataset)
        # --------------------------------
        ...
        np.savetxt(f'/data/dataset/csvs/x_y_logistic_{sample_size}_{client_id}.npy', merged)
        # --------------------------------
        rand_data, _ = make_classification(n_samples=10000, n_features=2, n_informative=2, n_redundant=0, n_classes=2,
                                           random_state=42 * client_id)
        np.savetxt(f'/data/dataset/csvs/x_y_corr_{sample_size}_{client_id}.npy', rand_data)