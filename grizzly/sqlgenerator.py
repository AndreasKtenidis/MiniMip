...
class SQLGenerator:
  ...
  @staticmethod
  def _generateCreateFunc(udf: UDF, templates, profile: str = None) -> str:
    # Early error handling for incorrect UDF type
    valid_types = {"native", "arrow", "threads", "multiprocess"}
    if udf.udf_type not in valid_types:
      raise ValueError(f"Invalid UDF type '{udf.udf_type}'. Expected one of {valid_types}.")

    if profile != "duckdb" and udf.udf_type in {"arrow", "threads"}:
      raise ValueError(f"UDF type '{udf.udf_type}' is not supported for profile '{profile}'.")

    # Parallel duckdb udf
    if profile == "duckdb" and (udf.udf_type == "threads" or udf.udf_type == "multiprocess"):
      
      pool_arguments_str = ""
      
      if udf.udf_type == "multiprocess":
        parallel_template = templates["createfunction_multiprocess"]
        if udf.num_workers is not None:
          pool_arguments_str = f"processes={udf.num_workers}"

      else:
        parallel_template = templates["createfunction_threads"]
        if udf.num_workers is not None:
          pool_arguments_str = f"max_workers={udf.num_workers}"
      
      if not parallel_template:
        raise ValueError("Missing 'createfunction_threads' or 'createfunction_multiprocess' template for parallel UDFs.")
      ...
      def get_param_names(signature):
        ...
        args_srt = match.group(1).strip()
        if not args_srt:
          return ()
        
        potential_args = args_srt.split(",")
        args_names = []
        for arg in potential_args:
          # Remove comment, default value, and type hint
          arg = arg.split('#')[0]
          arg = arg.split('=')[0]
          arg = arg.split(':')[0]

          # Handle *args and **kwargs (remove the '*' or '**')
          if arg.strip().startswith('**'):
            arg = arg.strip()[2:]
          elif arg.strip().startswith('*'):
            arg = arg.strip()[1:]
          
          # Clean whitespace and add if it's a valid identifier part
          cleaned_arg = arg.strip()
          if cleaned_arg:
            args_names.append(cleaned_arg)
        
        return tuple(args_names)
    
      # Get the parameter names from the function signature
      paramNames = get_param_names(udf.lines[0])
      params_arrow        = ",".join([f"{p}_arr" for p in paramNames])
      chunks              = ",".join([f"{p}_chunk" for p in paramNames]) + ","
      params_arrow_chunks = ",".join([f"{p}_arr.chunks" for p in paramNames])
      params_lists        = ",".join([f"{p}_list" for p in paramNames])
      empty_lists         = ",".join(["[]" for _ in paramNames])
      extend_lists        = ",".join([f"{p}_list.extend({p}_chunk.to_pylist())" for p in paramNames])

      # Create the inner code for the UDF, which is the original UDF code
      import textwrap
      first_line = udf.lines[0].replace(udf.name, f"{udf.name}_arrow", 1)
      first_line = textwrap.dedent(first_line)
      if len(udf.lines) > 1:
          rest_code = "\n".join(["\t" + line for line in udf.lines[1:]])
          inner_code = first_line + "\n" + rest_code
      else:
          inner_code = first_line

      lambda_args = ",".join([f"pair[{i}]" for i in range(len(paramNames))])
      lambda_code = f"lambda pair: {udf.name}_arrow({lambda_args})"

      # Replace the placeholders in the template with actual values
      newCode = parallel_template.replace("$$name$$", udf.name)\
        .replace("$$params_arrow$$", params_arrow)\
        .replace("$$chunks$$", chunks)\
        .replace("$$params_arrow_chunks$$", params_arrow_chunks)\
        .replace("$$params_lists$$", params_lists)\
        .replace("$$empty_lists$$", empty_lists)\
        .replace("$$extend_lists$$", extend_lists)\
        .replace("$$lambda_code$$", lambda_code)\
        .replace("$$code$$", inner_code)\
        .replace("$$pool_arguments$$", pool_arguments_str)\
    
      # Make it duckdb style with \x1F as seperator
      paramsStr  = ",".join([f"{SQLGenerator._mapTypes(p.type, templates['types'])}" for p in udf.params])
      returnType = returnType = SQLGenerator._mapTypes(udf.returnType, templates['types'])
      final_code = udf.name + "\x1F" + paramsStr + "\x1F" + returnType + "\x1F" + newCode + "\x1F" + udf.udf_type
      return final_code
    ...
    if isinstance(udf, ModelUDF):
      ...
      if udf.lang == "sql" and profile != "duckdb":
        try:
          # Try to compile code of udf and pass mapping template
          pre, lines = udfcompiler.compile(lines, templates, udf.params)
          if profile == "duckdb":
            # DuckDB does not support the creation of functions
            # so we need to use the function as a subquery
            lines = f"SELECT ({lines}) AS {udf.name};"
            return lines
    ...
    if profile == "duckdb":
      ...
      code = udf.name + "\x1F" + paramsStr + "\x1F" + returnType + "\x1F" + lines + "\x1F" + udf.udf_type
    else:
  ...
  def _generateCreateExtTable(tab: ExternalTable, templates) -> List[str]:
    ...
    is_duckdb = any("read_csv" in t for t in template)
    if is_duckdb:
      new_schema = []
      for entry in tab.colDefs:
        # Split the entry into individual definitions by comma.
        definitions = [d.strip() for d in entry.split(",") if d.strip()]
        for definition in definitions:
          parts = definition.split(":")
          if len(parts) == 2:
            colName = parts[0].strip()
            colType = parts[1].strip()
            mappedType = SQLGenerator._mapTypes(colType, templates["types"])
            new_schema.append(f"'{colName}':'{mappedType}'")
          else:
            new_schema.append(definition)
      schemaString = ",".join(new_schema)
    else:
      # Default as it was previously
    ...
    for t in template:
      code = t.replace("$$name$$", tab.table)\
        ...
        .replace("$$delimiter$$", tab.delimiter)\
        .replace("$$header$$", "true" if tab.hasHeader else "false")\
